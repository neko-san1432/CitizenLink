...
function initializeEventListeners() {
  ...
  document.querySelectorAll(".filter-btn").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      currentFilters.assignment_filter = target.dataset.filter;
      loadAssignments();
    });
  });
  ...
  document.getElementById("date-start")?.addEventListener("change", (e) => {
    currentFilters.date_start = e.target.value;
    loadAssignments();
  });

  document.getElementById("date-end")?.addEventListener("change", (e) => {
    currentFilters.date_end = e.target.value;
    loadAssignments();
  });
  ...
  // Modal close buttons
  document
    .getElementById("close-assignment-modal")
    ?.addEventListener("click", closeAssignmentModal);
  document
...
async function loadAssignments() {
  try {
    ...
    if (currentFilters.assignment_filter !== "all")
      params.append("assignment_filter", currentFilters.assignment_filter);
    if (currentFilters.status !== "all")
      params.append("status", currentFilters.status);
    if (currentFilters.sub_type !== "all")
      params.append("sub_type", currentFilters.sub_type);
    if (currentFilters.priority !== "all")
      params.append("priority", currentFilters.priority);
    if (currentFilters.date_start)
      params.append("date_start", currentFilters.date_start);
    if (currentFilters.date_end)
      params.append("date_end", currentFilters.date_end);

    const queryString = params.toString();
    const url = queryString
      ? `/api/lgu-admin/department-assignments?${queryString}`
      : "/api/lgu-admin/department-assignments";

    const response = await apiClient.get(url);

    if (response.success) {
      // Handle various response structures (data array or paginated object)
...
function updateStats() {
  // Note: Statistics might need to be fetched separately if we are paginating assignments
  // For now, we calculate based on loaded assignments, or we could fetch from dashboard-stats endpoint
...
function renderAssignments() {
  // Use assignments directly (already filtered by API)
  ...
  listEl.innerHTML = assignments
    .map((assignment) => createAssignmentHTML(assignment))
    .join("");

  // Attach event listeners to assign buttons
  assignments.forEach((assignment) => {
    const btn = document.querySelector(
      `[data-complaint-id="${assignment.complaint_id}"]`
    );
    if (btn) {
      btn.addEventListener("click", (e) => {
        e.preventDefault();
  ...
  return `
    ...
    assignment.submitted_at
        ...
        <div class="assignment-actions">
          <span class="priority-badge ${priorityClass}">${assignment.priority || "medium"
}</span>
...
function openAssignmentModal(assignment) {
  ...
  summaryEl.innerHTML = `
    ...
    <div class="complaint-summary-detail"><strong>Priority:</strong> ${assignment.priority || "medium"
}</div>
  `;
...
function populateOfficerSelect() {
  ...
  select.innerHTML = `<option value="">-- Select an officer --</option>${officers
    .map(
      (officer) => `
      <option value="${officer.id}">${escapeHtml(officer.name)} ${officer.employee_id ? `(${officer.employee_id})` : ""
}</option>
    `
    )
    .join("")}`;
