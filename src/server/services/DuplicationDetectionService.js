...
class DuplicationDetectionService {
  ...
  async detectDuplicates(complaintId) {
    try {
      ...
      const [textMatches, locationMatches, temporalMatches] = await Promise.all(
        [
      ...
      const merged = this.mergeAndScore(
        textMatches,
        locationMatches,
        temporalMatches
      );
  ...
  mergeAndScore(textMatches, locationMatches, temporalMatches) {
    ...
    // Combine all matches
    [...textMatches, ...locationMatches, ...temporalMatches].forEach(
      (match) => {
