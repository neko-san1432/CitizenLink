    ...
    try {
      ...
      complaints.forEach(complaint => {
        ...
        if (barangay && barangayData.has(barangay)) {
          ...
          console.warn(`[INSIGHTS] Complaint ${complaint.id} could not be classified into a barangay. Coords: (${complaint.latitude}, ${complaint.longitude})`);
        }
      });
      ...
      const { data: allHistoricalComplaints, error: allComplaintsError } = await this.supabase
        .from('complaints')
      ...
      if (allComplaintsError) {
        console.warn('[INSIGHTS] Error fetching historical complaints for averages:', allComplaintsError);
      }

      // Classify all historical complaints by barangay
      const allComplaintsBarangayMap = allHistoricalComplaints && allHistoricalComplaints.length > 0
      ...
      allComplaintsBarangayMap.forEach((barangayName, complaintId) => {
        ...
        const complaint = allHistoricalComplaints.find(c => c.id === complaintId);
        if (complaint) {
      ...
      const calculateAveragesForBarangay = (barangayName) => {
        ...
        const barangayComplaints = barangayHistoricalComplaints.get(barangayName) || [];
        ...
        const dailyCount = barangayComplaints.filter(c => new Date(c.submitted_at) >= oneDayAgo).length;
        const weeklyCount = barangayComplaints.filter(c => new Date(c.submitted_at) >= oneWeekAgo).length;
        const monthlyCount = barangayComplaints.filter(c => new Date(c.submitted_at) >= oneMonthAgo).length;
        const yearlyCount = barangayComplaints.filter(c => new Date(c.submitted_at) >= oneYearAgo).length;
      ...
      prioritizationData.sort((a, b) => b.prioritizationScore - a.prioritizationScore);
      ...
      console.error('[INSIGHTS_SERVICE] Get barangay prioritization error:', error);
      throw error;
    }
  }
