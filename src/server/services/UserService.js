...
class UserService {
  ...
  async updateUser(userId, updateData, updatedBy = null) {
    ...
    try {
      ...
      if (normalizedUpdateData.normalized_role) {
        normalizedUpdateData.normalized_role = normalizeRole(
          normalizedUpdateData.normalized_role
        );
      }
      if (normalizedUpdateData.base_role) {
        normalizedUpdateData.base_role = normalizeRole(
          normalizedUpdateData.base_role
        );
      }
      ...
      if (updatedMetadata.normalized_role) {
        updatedMetadata.normalized_role = normalizeRole(
          updatedMetadata.normalized_role
        );
      }
  ...
  async changeUserRole(userId, newRole, changedBy, reason = null) {
    ...
    const validRoles = [
      ...
      "lgu-admin-education",
      "lgu-admin-social",
      "lgu-admin-infrastructure",
      "lgu-admin-environment",
      "lgu-admin-agriculture",
      "lgu-admin-tourism",
      "lgu-admin-publicsafety",
      "lgu-admin-economic",
      "lgu-admin-legal",
      "lgu-hr-health",
      "lgu-hr-education",
  ...
  async getUsers(filters = {}, pagination = {}) {
    ...
    try {
      ...
      try {
        ...
        try {
          const pageAdjusted = page - 1;
          const fromPage = pageAdjusted * limit;
          const pageProd = page * limit;
          const toPage = pageProd - 1;
          ...
          const msg = error_?.message || sqlError_?.message || "Unknown error";
      ...
      if (role) {
        users = users.filter(
          (u) => u.role === role || u.normalizedRole === role
        );
      }
      if (status) {
  ...
  validateUserData(userData, isOAuth = false) {
    const { email, password, firstName, lastName, name, mobileNumber } =
      userData;
    if (!email?.includes("@")) {
  ...
  formatUserResponse(authUser) {
    ...
    const displayName =
      ...
      (combined.first_name || combined.last_name
        ? [combined.first_name, combined.middle_name, combined.last_name]
          .filter(Boolean)
          .join(" ")
          .trim()
        : "Unknown User");
    return {
      ...
      normalizedRole:
        combined.normalized_role || this.normalizeRole(combined.role),
