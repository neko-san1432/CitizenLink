  ...
  ErrorHandler.asyncWrapper(async (req, res) => {
    try {
      ...
      if (!anonKey || !supabaseUrl) {
        console.error(
          "[FORGOT_PASSWORD] Missing SUPABASE_ANON_KEY or SUPABASE_URL"
        );
        ...
        return res.status(500).json({
          ...
          details:
            process.env.NODE_ENV === "development"
              ? "Missing SUPABASE_ANON_KEY or SUPABASE_URL"
              : null,
        });
      }
      ...
      try {
        ...
        console.error(
          "[FORGOT_PASSWORD] Failed to create Supabase client:",
          clientError
        );
        return res.status(500).json({
          success: false,
...
router.post(
  ...
  ErrorHandler.asyncWrapper(async (req, res) => {
    try {
      ...
      } = require("../../shared/passwordValidation");
