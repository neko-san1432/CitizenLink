...
router.get('/citizen', authenticateUser, requireRole(['citizen']), (req, res) => {
  res.redirect('/dashboard');
});
router.get('/lgu', authenticateUser, requireRole(['lgu']), (req, res) => {
  res.redirect('/dashboard');
});
router.get('/coordinator', authenticateUser, requireRole(['complaint-coordinator']), (req, res) => {
  res.redirect('/dashboard');
});
router.get('/lgu-admin', authenticateUser, requireRole(['lgu-admin']), (req, res) => {
  res.redirect('/dashboard');
});

// HR specific pages (simplified URLs)
router.get('/link-generator',
  authenticateUser,
  requireRole(['lgu-hr']),
  (req, res) => {
...
router.get('/super-admin/link-generator',
  authenticateUser,
  requireRole(['super-admin']),
  (req, res) => {
    res.sendFile(path.join(config.rootDir, 'views', 'pages', 'hr', 'link-generator.html'));
...
router.get('/super-admin/pending-signups',
  authenticateUser,
  requireRole(['super-admin']),
  (req, res) => {
...
router.get('/super-admin/server-logs',
  authenticateUser,
  requireRole(['super-admin']),
  (req, res) => {
...
router.get('/coordinator/review/:id',
  authenticateUser,
  requireRole(['complaint-coordinator']),
  (req, res) => {
...
router.get('/lgu-officer/dashboard',
  authenticateUser,
  requireRole(['lgu']),
  (req, res) => {
    res.redirect('/dashboard');
...
router.get('/super-admin/role-changer',
  authenticateUser,
  requireRole(['super-admin']),
  (req, res) => {
    res.redirect('/super-admin/user-manager');
...
router.get('/super-admin/role-manager',
  authenticateUser,
  requireRole(['super-admin']),
  (req, res) => {
    res.redirect('/super-admin/user-manager');
...
router.get(['/oauth-continuation', '/oauthcontinuation'], (req, res) => {
...
router.get('/signup-with-code', (req, res) => {
