...
function createRoleButton(currentRole = 'lgu', baseRole = null) {
  ...
  let buttonText;
  if (isInCitizenMode) {
    const roleName = baseRole ? baseRole.toUpperCase() : 'STAFF';
    buttonText = `👤 Switch to ${roleName} View`;
    console.log('[ROLE_TOGGLE] In citizen mode, will switch back to:', baseRole);
  } else {
    buttonText = '👤 Switch to Citizen View';
    console.log('[ROLE_TOGGLE] In staff mode, will switch to citizen');
  }
...
export async function initializeRoleToggle() {
  ...
  try {
    ...
    // Show button if user is not a pure citizen (has any staff role or is in citizen mode with a base role)
    if (userRole === 'citizen' && !baseRole) {
      return;
    }
