  ...
  for (const trigger of SPECULATIVE_TRIGGERS) {
    ...
    if (trigger.endsWith("-")) {
      ...
      if (wordBeforeKeyword === prefix || preContext.endsWith(prefix)) {
        return { isSpeculative: true, trigger, position: "before", type: "prefix" };
      }
    } else if (preContext.includes(trigger)) {
      return { isSpeculative: true, trigger, position: "before", type: "conditional" };
    }
  }
...
class SimulationEngine {
  ...
  async initialize() {
    try {
      this.addLog("[SYSTEM] Loading mockcomplaints.json...", "system");
      ...
      // V3.9 UPDATE: Mock data loading removed. Data is now injected via dashboardProduction.js
      ...
      // Stats will be updated when data arrives via dashboardProduction.js
      this.updateStats({
  ...
  async loadBarangayBoundaries() {
    try {
      ...
      const possiblePaths = [
        "brgyBoundariesLocation.json",                    // Root directory (actual location)
        "data/geographic/brgyBoundariesLocation.json",    // Subdirectory
        "../assets/data/geographic/brgyBoundariesLocation.json", // From src subdirectory
        "digos-city-boundary.json",                         // Alternative file
        "/assets/json/brgyBoundariesLocation.json",
        "/assets/json/digosCityBoundary.json",
        "/assets/data/geographic/brgyBoundariesLocation.json"    // Absolute from root
      ];
      ...
      if (!barangays) {
        throw new Error("Could not find brgyBoundariesLocation.json");
      }
  ...
  createRejectMarker(lat, lng) {
    ...
    const marker = L.marker([lat, lng], { icon, zIndexOffset: 900 }).addTo(this.map);
    this.spotlightMarkers.push(marker);
    return marker;
  }
...
async function loadNLPDictionaries(forceReload = false) {
  ...
  try {
    ...
    const possiblePaths = [
      ...
      "/assets/data/nlp_dictionaries.json",                  // Absolute from server root (most reliable)
      "data/nlp_dictionaries.json",                   // From root
      "../../assets/data/nlp_dictionaries.json",             // From src/dashboard or src/analytics
      "../assets/data/nlp_dictionaries.json",                // From src folder
...
const NLP_SEVERITY_AMPLIFIERS = new Map();     // term -> multiplier
const NLP_SEVERITY_DIMINISHERS = new Map();    // term -> multiplier
