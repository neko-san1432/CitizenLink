...
export function initializeNotificationButton() {
  ...
  if (notificationBtn && notificationPanel) {
    ...
    notificationBtn.addEventListener("click", (e) => {
      ...
      // Debug positioning
      if (notificationPanel.classList.contains("show")) {
        notificationPanel.getBoundingClientRect();
      }
  ...
  try {
    ...
    if (response?.success) {
      ...
      if (connectionErrorCount >= 3) {
        console.warn(
          "[NOTIFICATION] Multiple connection errors detected. Stopping polling."
        );
        stopPolling();
...
async function checkForNewNotifications() {
  try {
    ...
    console.error(
      "[NOTIFICATION] Failed to check for new notifications:",
      error
    );
  }
}
    ...
    if (response.success) {
      ...
      if (reset) {
        ...
        const existingIds = new Set(
          notificationState.notifications.map((n) => n.id)
        );
...
async function fetchNotifications(page, limit) {
  try {
    const response = await apiClient.get(
      `/api/notifications/unread?limit=${limit}&offset=${page * limit}`
    );
    if (!response.success) {
...
function renderNotifications() {
  ...
  const html = notificationState.notifications
    .map((notification, index) =>
      createNotificationItemHtml(notification, index)
    )
    .join("");
...
function createNotificationItemHtml(notification, index) {
  let priorityClass = "";
  ...
  return `
    <div class="notification-item ${readClass} ${priorityClass} ${newClass}" 
...
function updateShowMoreButton() {
  ...
  if (notificationState.hasMore) {
    ...
    showMoreBtn.textContent = notificationState.loading
      ? "Loading..."
      : "Show More";
