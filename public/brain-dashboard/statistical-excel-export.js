/**
 * DRIMS Statistical Analysis Excel Export Module
 * =====================================================
...
async function exportStatisticalExcel() {
  ...
  try {
    ...
    const filename = `DRIMS_Statistical_Report_${timestamp}.xlsx`;
...
function createStatsSummarySheet(stats, rawData) {
  const data = [
    ["DRIMS STATISTICAL ANALYSIS REPORT"],
...
function createCategoryPerformanceSheet(rawData) {
  ...
  catList.forEach(cat => {
    ...
    metrics[cat] = { tp, fp, fn, tn, precision, recall, f1 };
  });
...
function createStatisticalTestsSheet(stats) {
  const tStat = formatNum(stats.ttest ? stats.ttest.t_statistic : null, 4);
  const pVal = formatNum(stats.ttest ? stats.ttest.p_value : null, 6);
  const cohenD = formatNum(stats.ttest ? stats.ttest.cohens_d : null, 4);
  const chiSq = formatNum(stats.chi_square ? stats.chi_square.chi_square : null, 4);
  const chiP = formatNum(stats.chi_square ? stats.chi_square.p_value : null, 6);
  const cramerV = formatNum(stats.chi_square ? stats.chi_square.cramers_v : null, 4);

  const tDecision = (stats.ttest && stats.ttest.p_value < 0.05) ? "SIGNIFICANT (p < 0.05) - Reject null hypothesis" : "Not significant";
  const chiDecision = (stats.chi_square && stats.chi_square.p_value < 0.05) ? "SIGNIFICANT - Categories are correlated" : "Not significant";
...
function createCategoryDistributionSheet(rawData) {
  ...
  const tiers = { "Tier 1 - CRITICAL": 0, "Tier 2 - MODERATE": 0, "Tier 3 - LOW": 0 };
  complaints.forEach(c => {
